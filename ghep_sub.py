...
HIGH_CONFIDENCE_THRESHOLD = (
    0.75  # Chỉ match có similarity >= giá trị này mới cập nhật last_matched anchor
)

# Index Jump Penalty - Phạt khi nhảy cóc quá nhiều dòng (chỉ áp dụng khi similarity không đủ cao)
INDEX_JUMP_PENALTY_FACTOR = (
    0.01  # Penalty cho mỗi dòng nhảy cóc (0.01 = 1% penalty per line - nhẹ hơn)
)
MAX_INDEX_JUMP_PENALTY = 0.2  # Penalty tối đa cho index jump
INDEX_JUMP_FREE_THRESHOLD = (
    5  # Cho phép nhảy 5 dòng mà không bị phạt (do có thể thiếu dòng)
)
INDEX_JUMP_DISABLE_SIMILARITY = (
    0.70  # Nếu similarity >= giá trị này, bỏ qua index jump penalty
)
...
def split_subtitle_entry(entry: SubtitleEntry, base_index: int) -> List[SubtitleEntry]:
    """
...
def apply_pre_splitting(
    entries: List[SubtitleEntry], entry_type: str = "unknown"
) -> List[SubtitleEntry]:
    """
    ...
    print(
        f"    After split: {len(virtual_entries)} entries ({split_count} entries were split)"
    )
...
class TimeOffsetDetector:
    ...
    def detect_global_offset(
        self, en_entries: List[SubtitleEntry], vi_entries: List[SubtitleEntry]
    ) -> float:
        """
        ...
        for idx in sample_indices:
            offset, sim = self._find_best_offset_for_sample(en_entries[idx], vi_entries)
    ...
    def apply_offset(
        self, vi_entries: List[SubtitleEntry], offset: float
    ) -> List[SubtitleEntry]:
        """
    ...
    def auto_detect_and_apply(
        ...
        """
        info = {"global_offset": 0.0, "piecewise_offset": 0.0, "method": "none"}
...
class SubtitleMatcher:
    ...
    def _build_context(self, entries: List[SubtitleEntry], idx: int) -> str:
        """
    ...
    def _precompute_embeddings(
        ...
        if CONTEXT_WEIGHT > 0:
            ...
            en_ctx_valid = [
                (i, ctx) for i, ctx in zip(en_text_indices, en_contexts) if ctx
            ]
            vi_ctx_valid = [
                (i, ctx) for i, ctx in zip(vi_text_indices, vi_contexts) if ctx
            ]
        ...
        print(
            f"  ✓ Pre-computed {len(self._en_embeddings)} EN + {len(self._vi_embeddings)} VI embeddings"
        )

    def _calculate_time_penalty(
        self, en_entry: SubtitleEntry, vi_entry: SubtitleEntry
    ) -> float:
        """
    ...
    def _calculate_order_penalty(
        self, vi_entry: SubtitleEntry, en_entry: SubtitleEntry
    ) -> float:
        """
    ...
    def _calculate_index_jump_penalty(
        self, vi_entry: SubtitleEntry, similarity: float
    ) -> float:
        """
    ...
    def _get_similarity_score(self, en_idx: int, vi_idx: int) -> float:
        """
        ...
        if self.model is not None:
            ...
            if CONTEXT_WEIGHT > 0:
                ...
                if en_ctx_emb is not None and vi_ctx_emb is not None:
                    context_score = float(
                        np.dot(en_ctx_emb, vi_ctx_emb)
                        / (
                            np.linalg.norm(en_ctx_emb) * np.linalg.norm(vi_ctx_emb)
                            + 1e-9
                        )
                    )
                    return (
                        1 - CONTEXT_WEIGHT
                    ) * text_score + CONTEXT_WEIGHT * context_score
    ...
    def find_best_match(
        ...
        if (
            en_entry.index not in self._en_embeddings
            and en_entry.index not in self._en_normalized
        ):
            return None
        ...
        for vi_idx, vi_entry in enumerate(vi_entries):
            ...
            if en_duration > 0 and vi_duration > 0:  # Tránh chia cho 0
                duration_ratio = max(en_duration, vi_duration) / min(
                    en_duration, vi_duration
                )
            ...
            if (
                vi_entry.index not in self._vi_embeddings
                and vi_entry.index not in self._vi_normalized
            ):
                continue
    ...
    def _detect_split_sentences(
        self, en_entries: List[SubtitleEntry]
    ) -> List[Tuple[int, int]]:
        """
    ...
    def match_subtitles(
        ...
        """
        Match tất cả các dòng subtitle sử dụng Dynamic Time Warping (DTW)

        DTW đảm bảo:
        1. Timeline không bao giờ nhảy lùi
        2. Tối ưu hóa toàn cục (không phải tham lam từng dòng)
        3. Cho phép skip entries khi không match được
        ...
        print(f"Mode: {self.mode} | Using DTW (Dynamic Time Warping)")
        print(f"Min Cosine: {self.min_cosine:.2f} | Min Fuzz: {self.min_fuzz}")
        ...
        cost_matrix = self._build_cost_matrix(en_entries, vi_entries)

        # Chạy DTW
        print("  Running DTW algorithm...")
        matches = self._dtw_match(en_entries, vi_entries, cost_matrix)

        # Thống kê
        matched = sum(1 for _, vi, _ in matches if vi is not None)
        print(
    ...
    def _build_cost_matrix(
        ...
        """
        Xây dựng ma trận cost để match en[i] với vi[j]

        Cost = 1 - similarity + time_penalty + duration_penalty
        Cost thấp = match tốt

        Returns:
            cost_matrix[i][j]: cost để match en_entries[i] với vi_entries[j]
        """
        n_en = len(en_entries)
        n_vi = len(vi_entries)

        cost_matrix = [[float("inf")] * n_vi for _ in range(n_en)]

        for i, en_entry in enumerate(en_entries):
            if (i + 1) % 100 == 0:
                print(f"    Building cost matrix: {i + 1}/{n_en}")
            ...
            for j, vi_entry in enumerate(vi_entries):
                # Tính similarity
                similarity = self._get_similarity_score(en_entry.index, vi_entry.index)

                # Kiểm tra threshold: nếu similarity quá thấp, skip
                ...
                en_duration = en_entry.end - en_entry.start
                vi_duration = vi_entry.end - vi_entry.start
                if en_duration > 0 and vi_duration > 0:
                    duration_ratio = max(en_duration, vi_duration) / min(
                        en_duration, vi_duration
                    )
                    if duration_ratio > MAX_DURATION_RATIO:
                ...
                # Cost = 1 - similarity + penalties (cost càng thấp càng tốt)
                cost = (1.0 - similarity) + time_penalty + duration_penalty
    ...
    def _dtw_match(
        ...
        """
        Dynamic Time Warping để tìm alignment tối ưu

        dp[i][j] = minimum total cost để match en[0..i-1] với vi[0..j-1]

        Transitions:
        - dp[i-1][j] + skip_cost: skip EN entry i-1 (không match)
        - dp[i][j-1] + skip_cost: skip VI entry j-1 (không match)
        - dp[i-1][j-1] + cost_matrix[i-1][j-1]: match en[i-1] với vi[j-1]

        Returns:
            List of matches: [(en_entry, vi_entry or None, score), ...]
        """
        n_en = len(en_entries)
        n_vi = len(vi_entries)

        # Cost để skip một entry (không match)
        SKIP_COST = 0.5

        # DP table: dp[i][j] = minimum cost to align en[0:i] with vi[0:j]
        INF = float("inf")
        dp = [[INF] * (n_vi + 1) for _ in range(n_en + 1)]

        # Backtrack table để trace path
        # 0: match, 1: skip en, 2: skip vi
        backtrack = [[None] * (n_vi + 1) for _ in range(n_en + 1)]

        # Base case
        dp[0][0] = 0

        # Initialize first row: skip all VI entries
        for j in range(1, n_vi + 1):
            dp[0][j] = dp[0][j - 1] + SKIP_COST
            backtrack[0][j] = 2  # skip vi

        # Initialize first column: skip all EN entries
        for i in range(1, n_en + 1):
            dp[i][0] = dp[i - 1][0] + SKIP_COST
            backtrack[i][0] = 1  # skip en

        # Fill DP table
        for i in range(1, n_en + 1):
            for j in range(1, n_vi + 1):
                # Option 1: Match en[i-1] with vi[j-1]
                match_cost = cost_matrix[i - 1][j - 1]
                if match_cost != INF:
                    cost_match = dp[i - 1][j - 1] + match_cost
                else:
                    cost_match = INF

                # Option 2: Skip en[i-1]
                cost_skip_en = dp[i - 1][j] + SKIP_COST

                # Option 3: Skip vi[j-1]
                cost_skip_vi = dp[i][j - 1] + SKIP_COST

                # Take minimum
                min_cost = min(cost_match, cost_skip_en, cost_skip_vi)
                dp[i][j] = min_cost

                if min_cost == cost_match:
                    backtrack[i][j] = 0  # match
                elif min_cost == cost_skip_en:
                    backtrack[i][j] = 1  # skip en
                else:
                    backtrack[i][j] = 2  # skip vi

        # Backtrack để lấy alignment
        print("  Backtracking optimal path...")
        alignment = []  # List of (en_idx, vi_idx or None)
        i, j = n_en, n_vi
        ...
        # Reverse alignment (đã backtrack từ cuối về đầu)
        alignment.reverse()
        ...
        for en_idx, vi_idx in alignment:
            en_entry = en_entries[en_idx]

            if vi_idx is not None:
                vi_entry = vi_entries[vi_idx]
                vi_used.add(vi_idx)

                # Tính similarity score (1 - cost)
                raw_similarity = self._get_similarity_score(
                    en_entry.index, vi_entry.index
                )
                results.append((en_entry, vi_entry, raw_similarity))
            else:
                results.append((en_entry, None, 0.0))
...
def sync_subtitles(en_path: str, vi_path: str, output_path: str, mode: str = "auto"):
    """
    ...
    if "piecewise_offset" in offset_info and offset_info["piecewise_offset"] != 0:
        print(
            f"  Piecewise offset (first lines): {offset_info['piecewise_offset']:.2f}s"
        )
